...
/**
    Configure: configure as follows `Apis.instance("wss://dex.eidos.one").init_promise`.  This returns a promise, once resolved the connection is ready.
...
class Apis {
    ...
    */
    static reset( cs = "wss://dex.eidos.one", connect, connectTimeout = 4000, optionalApis, closeCb ) {
    ...
    static instance( cs = "wss://dex.eidos.one", connect, connectTimeout = 4000, optionalApis, closeCb) {
        if ( ! inst ) {
            inst = new Apis();
            inst.setRpcConnectionStatusCallback(this.statusCb);
        }
